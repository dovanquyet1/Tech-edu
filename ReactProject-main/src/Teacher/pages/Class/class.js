...
const TClass = () => {
  ...
  const handleCreate = async (e) => {
    ...
    const data = input
    const newClass = [...dataClases]
    await newClass.push({
      teacherImg: 'https://avatars.preply.com/i/logos/i/logos/avatar_wslh02bek4c.jpg',
      teacherName: 'Nguyen Minh Duc',
      id: data.id,
      name: data.name,
      img: data.img
    })
    setdataClases(newClass)
        ...
        {dataClases === undefined || dataClases.map((classItem, index) => {
            return (
              <div onClick={(e) => {
                ...
                return(
                  setTimeout(() => {
                    window.location.replace(`http://localhost:3001/detail/${index+1}`)
                  },2000)
